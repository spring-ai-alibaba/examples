...
- 启动入口：BigToolApplication
- Web 入口：BigToolController
- 核心服务：VectorStoreService
- 模型依赖：DashScope，通常需要配置 `DASHSCOPE_API_KEY` 或 `AI_DASHSCOPE_API_KEY`。

## 运行方式

```bash
mvn -f spring-ai-alibaba-graph-example/big-tool/pom.xml test-compile -DskipTests
mvn -f spring-ai-alibaba-graph-example/big-tool/pom.xml spring-boot:run
```
