...
- 启动入口：RagAutoConfigureExampleApplication
- Web 入口：RagAutoConfigureController
...
```bash
mvn -f spring-ai-alibaba-rag-example/rag-elasticsearch-autoconfigure-example/pom.xml test-compile -DskipTests
mvn -f spring-ai-alibaba-rag-example/rag-elasticsearch-autoconfigure-example/pom.xml spring-boot:run
```
