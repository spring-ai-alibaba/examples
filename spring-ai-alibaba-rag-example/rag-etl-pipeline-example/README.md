...
- 启动入口：RagEtlPipelineApplication
- Web 入口：ReaderController、TransformerController、WriterController
- 模型依赖：DashScope，通常需要配置 `DASHSCOPE_API_KEY` 或 `AI_DASHSCOPE_API_KEY`。

## 运行方式

```bash
mvn -f spring-ai-alibaba-rag-example/rag-etl-pipeline-example/pom.xml test-compile -DskipTests
mvn -f spring-ai-alibaba-rag-example/rag-etl-pipeline-example/pom.xml spring-boot:run
```
