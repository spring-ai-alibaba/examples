...
- 启动入口：BailianKnowledgeRagExampleApplication
- Web 入口：CloudRagController
- 核心服务：CloudRagService、RagService
- 模型依赖：DashScope，通常需要配置 `DASHSCOPE_API_KEY` 或 `AI_DASHSCOPE_API_KEY`。

## 运行方式

```bash
mvn -f spring-ai-alibaba-rag-example/bailian-rag-knowledge/pom.xml test-compile -DskipTests
mvn -f spring-ai-alibaba-rag-example/bailian-rag-knowledge/pom.xml spring-boot:run
```
