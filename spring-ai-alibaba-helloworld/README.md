# Spring-Ai-Alibaba-Helloworld 模块

## 模块说明

本模块是 spring-ai-alibaba-helloworld 模块，包含 1 个控制器。
...
**功能描述：** ChatClient 使用自定义的 Advisor 实现功能增强.
...
**示例请求：**
```bash
GET http://localhost:8080/helloworld/advisor/newChat
```
...
### 使用 HTTP 文件测试
模块根目录下提供了 **[spring-ai-alibaba-helloworld.http](./spring-ai-alibaba-helloworld.http)** 文件，包含所有接口的测试用例：
- 可在 IDE 中直接执行
- 支持参数自定义
- 提供默认示例参数
...
```bash
# simpleChat 接口测试
curl "http://localhost:8080/helloworld/simple/chat"
```
