...
- 子模块：ai-mcp-github、ai-mcp-fileserver、sqlite/ai-mcp-sqlite、sqlite/ai-mcp-sqlite-chatbot
- 用途：作为同类示例的 Maven 聚合入口，便于统一编译和按需进入子模块运行。

## 运行方式

```bash
mvn -f spring-ai-alibaba-mcp-example/spring-ai-alibaba-mcp-manual-example/pom.xml test-compile -DskipTests
