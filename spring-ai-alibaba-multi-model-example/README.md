# Spring AI Alibaba Multi Model Examples
...
- 子模块：dashscope-multi-model、ark-multi-model、openai-dashscope-multi-model
- 用途：作为同类示例的 Maven 聚合入口，便于统一编译和按需进入子模块运行。

## 运行方式

```bash
mvn -f spring-ai-alibaba-multi-model-example/pom.xml test-compile -DskipTests
