...
本模块演示 ReAct Agent，把模型推理、工具选择和执行结果组织成多轮决策流程。

## 主要内容

- 启动入口：Application
- Web 入口：AgentController
- 模型依赖：DashScope，通常需要配置 `DASHSCOPE_API_KEY` 或 `AI_DASHSCOPE_API_KEY`。

## 运行方式

```bash
mvn -f spring-ai-alibaba-agent-example/react-agent-example/pom.xml test-compile -DskipTests
mvn -f spring-ai-alibaba-agent-example/react-agent-example/pom.xml spring-boot:run
```
